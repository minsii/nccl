...
ncclResult_t ncclWinAllocShared(size_t size, ncclComm_t comm, ncclWin_t* win) {
  ...
  for (int i = 0; i < nRanks; ++i) {
    ...
    INFO(
        NCCL_INIT,
        "Rank %d Opened IPC handle for rank %d with remoteAddr %p in ncclWin %p on ncclComm %p (nNodes=%d, nRanks=%d, localRanks=%d, commHash=%ld)",
        comm->rank,
        i,
        remoteAddr,
        win_,
...
ncclResult_t ncclWinFree(ncclComm_t comm, ncclWin_t win) {
  INFO(
        NCCL_INIT,
        "Rank %d freeing ncclWin %p on ncclComm %p (nNodes=%d, nRanks=%d, localRanks=%d, commHash=%ld)",
        comm->rank,
        win,
        comm,
        comm->nNodes,
