...
class ProxyTrace {
 public:
  ...
  ProxyTrace::Dump dump(uint64_t commHash);
...
#define PROXY_TRACE_INFO_COPY(proxyOp, info)                   \
  do {                                                         \
    proxyOp.traceArgs.collInfo.commHash = info.comm->commHash; \
    proxyOp.traceArgs.collInfo.opCount = info.comm->opCount;   \
    proxyOp.traceArgs.collInfo.coll = info.coll;               \
    proxyOp.traceArgs.collInfo.nChannels = info.nChannels;     \
    proxyOp.traceArgs.rank = info.comm->rank;                  \
  } while (0)
...
#define PROXY_TRACE_OP_UPDATE_REMOTE_RANK(op, peer) \
  do {                                              \
    (op)->traceArgs.remoteRank = (peer);            \
  } while (0)
#endif
